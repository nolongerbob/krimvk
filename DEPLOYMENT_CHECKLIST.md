...
- [ ] Создать `.env` файл по шаблону `.env.example.vps`
...
### 11. CI/CD
- [ ] Добавить GitHub Secrets по `docs/GITHUB_SECRETS.md` (prod + dev)
- [ ] Создать каталоги `/var/www/krimvk` и `/var/www/krimvk-dev`
- [ ] Настроить `.env` (prod) и `.env.dev` (dev) на сервере
- [ ] Проверить CI на PR в `main` и `develop`
- [ ] Проверить деплой на push в `develop` (dev, порт 3001)
- [ ] Проверить деплой на push в `main` (prod, порт 3000)
- [ ] Прочитать `docs/CI_CD.md`

### 12. Требования РФ (152-ФЗ/149-ФЗ)
- [ ] Опубликовать `/legal/privacy`
- [ ] Опубликовать `/legal/terms`
- [ ] Опубликовать `/legal/cookies`
- [ ] Проверить показ cookie consent баннера и фиксацию выбора
- [ ] Проверить локализацию хранения/первичной обработки ПДн
- [ ] Назначить ответственного за ПДн и регламент удаления/выгрузки данных
- [ ] Проверить runbook `docs/PRODUCTION_RUNBOOK.md` и процесс инцидентов
