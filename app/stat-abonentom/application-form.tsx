...
export function ApplicationForm({ formData, isPreview = false }: { formData: any; isPreview?: boolean }) {
  ...
  const fontSize = isPreview ? "11px" : "10.5pt"; // Уменьшили для умещения на 2 листа
  const fontSizeLarge = isPreview ? "19px" : "17pt"; // Уменьшили заголовок пропорционально
  ...
  return (
    <div className={containerClass} style={{ fontFamily: "Times New Roman, serif", lineHeight: "1.5", fontSize: fontSize, ...containerStyle }}>
      ...
      {/* Дата и подпись */}
      <p style={{ margin: 0, textAlign: "justify", textIndent: 0, fontSize: fontSize, display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
