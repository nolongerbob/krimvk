      ...
      <Link href={href} className="block">
        <Card className={`flex flex-col cursor-pointer group h-full transition-all ${
          ...
          <CardHeader className="pb-4 flex-1">
            <Icon className={`${isEmergency ? 'h-12 w-12 text-red-600' : `h-10 w-10 ${iconColor}`} mb-3 transition-transform duration-500 ease-out group-hover:scale-110`} />
            <CardTitle className="mb-2">{title}</CardTitle>
            <CardDescription className="mb-0">{description}</CardDescription>
          </CardHeader>
          <CardContent className="pt-0 pb-6">
            <Button asChild className={`w-full pointer-events-none ${isEmergency ? 'bg-red-600 hover:bg-red-700 text-white' : ''}`}>
              <span>{title}</span>
            </Button>
          </CardContent>
        </Card>
  ...
  return (
    <Link href={href} className="block">
      <Card className={`flex flex-col cursor-pointer group h-full transition-all ${
        ...
        <CardHeader className="pb-4 flex-1">
          <Icon className={`${isPrimary ? 'h-12 w-12 text-blue-600' : `h-10 w-10 ${iconColor}`} mb-3 transition-transform duration-500 ease-out group-hover:scale-110`} />
          <CardTitle className="mb-2">{title}</CardTitle>
          <CardDescription className="mb-0">{description}</CardDescription>
        </CardHeader>
        <CardContent className="pt-0 pb-6">
          <Button asChild className={`w-full pointer-events-none ${isPrimary ? 'bg-blue-600 hover:bg-blue-700 text-white' : ''}`}>
            <span>{title}</span>
          </Button>
        </CardContent>
      </Card>
